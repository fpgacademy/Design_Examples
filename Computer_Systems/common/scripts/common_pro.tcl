# The line below must be included exactly as given
package require -exact qsys 15.0
#
#
# This script is used to build Computer Systems with Platform Designer, for University
# Program lab boards that used Agilex 3/5 devices. This script is written for Quartus Prime 
# Pro and calls functions defined in Computer_Systems/common/common_pro.tcl. Some functions 
# in this script are legacy, not used for Agilex 3/5 boards, and are left in this file only 
# as examples for possible future use.
...
set ifup_flag_set_fpga_data_masters			[ expr $ifup_flag_jtag_to_fpga_bridge | $ifup_flag_niosV_0_data_manager | $ifup_flag_niosV_1_data_manager ]
set ifup_flag_set_fpga_instruction_masters	[ expr $ifup_flag_niosV_0_instruction_manager | $ifup_flag_niosV_1_instruction_manager ]
set ifup_flag_set_fpga_peripherals			[ expr $ifup_flag_set_fpga_data_masters | $ifup_flag_hps_h2f_lw_master ]
#
# Avalon irq receiver types
set ifup_flag_set_main_irq_receivers		[ expr $ifup_flag_hps_arm_0_irq_receiver | $ifup_flag_niosV_0_irq_receiver | $ifup_flag_niosV_1_irq_receiver ]
###############################################################################
...
#
#used for the core_sdram_axi4 IP core from OpenCores
proc ifup_connect_sdram_sample_clock { clock_sink } {
	global sdram_sample_clock
	add_connection $sdram_sample_clock $clock_sink
}
#
...
# Clock Bridge: used for Agilex 3/5 boards
#     Used to receive the incoming system clock, called $export_name, and provide a global
#     signal, named system_clock, that can be connected in Platform Designer to any IP core 
#     that needs to use this system clock signal
#
proc ifup_add_system_clock_bridge {} {
	global system_clock

	set instance_name clock_in
	set export_name system

	add_instance $instance_name altera_clock_bridge
	set_interface_property $export_name	EXPORT_OF $instance_name.in_clk
	set system_clock $instance_name.out_clk
}
#
# Clock Bridge: used for Agilex 3 board
#     Used to receive the incoming SDRAM sample clock, called $export_name, and provide a
#     global signal, named sdram_sample_clock, that can be connected in Platform Designer to 
#     the sdram IP core that needs to use this sample clock signal
#
proc ifup_add_sdram_sample_clock_bridge {} {
    global sdram_sample_clock
	set instance_name sdram_sample_clock_in
	set export_name sdram_sample

	add_instance $instance_name altera_clock_bridge
	set_interface_property $export_name	EXPORT_OF $instance_name.in_clk
	set sdram_sample_clock $instance_name.out_clk
}
#
# Clock Bridge: used for Agilex 3/5 boards
#     Used to receive the incoming VGA clock, called $export_name, and provide a
#     global signal, named vga_clock, that can be connected in Platform Designer to 
#     the vga subsystem
#
proc ifup_add_vga_clock_bridge {} {
	global video_vga_clock
	set instance_name vga_clock_in
	set export_name vga_in

	add_instance $instance_name altera_clock_bridge
	set_interface_property $export_name	EXPORT_OF $instance_name.in_clk
	set video_vga_clock $instance_name.out_clk
}
#
# Reset Bridge: used for Agilex 3/5 boards
#     Used to distribute the global reset signal for any IP core that needs it
proc ifup_add_reset_bridge { } {
	global reset_sources
	set instance_name reset_in
	set export_name system_reset
	add_instance $instance_name altera_reset_bridge

	ifup_connect_system_clock $instance_name.clk
	set_interface_property $export_name EXPORT_OF $instance_name.in_reset
	lappend reset_sources $instance_name.out_reset
}
...
#
# Arm A9: used for pre-Agilex boards. Have to add new version for Agilex 5 boards
...
proc ifup_add_niosVg { instance_name data_master_flag instruction_master_flag irq_receiver_flags } {
	...
	set_instance_parameter_value $instance_name resetSlave               "Absolute"
	set_instance_parameter_value $instance_name resetOffset              0
	set_instance_parameter_value $instance_name peripheralRegionABase    0xFF000000
	set_instance_parameter_value $instance_name peripheralRegionASize    0x1000000
	set_instance_parameter_value $instance_name peripheralRegionBBase	 0x8000000
	set_instance_parameter_value $instance_name peripheralRegionBSize    0x8000000
...
#
# Used for DE23-Lite
proc ifup_add_sdram_64mb_32bit {} {
	global ifup_flag_hps_h2f_master
	global ifup_flag_set_fpga_data_masters
	global ifup_flag_set_fpga_instruction_masters
	...
	add_instance $instance_name core_sdram_axi4

	set_interface_property sdram	EXPORT_OF $instance_name.sdram

	ifup_connect_system_clock			$instance_name.clock_sink
	ifup_connect_sdram_sample_clock		$instance_name.clock_sample_sink
	ifup_connect_reset_sources			$instance_name.reset_sink
	ifup_connect_slave_to_mm_masters	$instance_name.axi4_slave	 $offset $bus_connection_flags

	lock_avalon_base_address $instance_name.axi4_slave
	
	# Update connection lists
	global memory_mapped_slaves
	global ifup_flag_video_in_memory
	global ifup_flag_video_out_memory
	set memory_mapped_slaves($instance_name.axi4_slave) [ list [ expr $ifup_flag_video_in_memory | $ifup_flag_video_out_memory ] $offset ]
}
#
# SRAM
proc ifup_add_sram {} {
...
#
# Onchip Memory
proc ifup_add_onchip_memory_II { instance_name memory_size offset } {
	global ifup_flag_hps_h2f_master
	global ifup_flag_set_fpga_data_masters
	global ifup_flag_set_fpga_instruction_masters

	add_instance $instance_name intel_onchip_memory

	set_instance_parameter_value $instance_name AXI_interface			1
	set_instance_parameter_value $instance_name dataWidth			    32
	set_instance_parameter_value $instance_name dataWidth2			    32
	set_instance_parameter_value $instance_name dualPort				2
	set_instance_parameter_value $instance_name memorySize				$memory_size
	set_instance_parameter_value $instance_name singleClockOperation	1
...
proc ifup_add_vga_subsystem {} {
	...
	ifup_connect_reset_sources	        $instance_name.vga_reset
